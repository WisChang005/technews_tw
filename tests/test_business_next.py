...
from libs import business_next


@pytest.fixture
def news_object():
    return business_next.BusinessNext()


@pytest.mark.parametrize("browser_page", [0, 1, 3, 5])
def test_business_next_page_response(news_object, browser_page):
...
def test_business_next_page_load_verification(news_object):
