...
logging.basicConfig(level=logging.INFO, format=strFormat)
...
def test_tech_orange_page_response(tech_orange, browser_page):
    news_data = tech_orange.get_news(browser_page)
    _news_data_contents_assertion(news_data)


def test_tech_orange_page_load_verification(tech_orange):
    page1_data = tech_orange.get_news()
    page_more_data = tech_orange.get_news(MORE_PAGE_TEST_INDEX)
    _page_load_assertion(page1_data, page_more_data)


@pytest.mark.parametrize("browser_page", [0, 1, 3, 5])
def test_ithome_page_response(ithome, browser_page):
    news_data = ithome.get_news(browser_page)
    _news_data_contents_assertion(news_data)


def test_ithome_page_load_verification(ithome):
...
def test_business_next_page_response(business_next, browser_page):
    news_data = business_next.get_news(browser_page)
    _news_data_contents_assertion(news_data)


def test_business_next_page_load_verification(business_next):
    page1_data = business_next.get_news()
    page_more_data = business_next.get_news(MORE_PAGE_TEST_INDEX)
    _page_load_assertion(page1_data, page_more_data)


def _news_data_contents_assertion(news_data):
...
def _page_load_assertion(page1, page_more):
